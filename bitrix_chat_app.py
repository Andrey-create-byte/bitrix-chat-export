import requests import json import os import shutil import streamlit as st from zipfile import ZipFile from datetime import datetime
...
def get_chat_messages(dialog_id, limit=200, offset=0): url = f"{WEBHOOK_URL}im.dialog.messages.get.json" payload = { "DIALOG_ID": dialog_id, "LIMIT": limit, "OFFSET": offset } response = requests.post(url, json=payload) return response.json()

Функция для получения списка недавних чатов

def get_recent_chats(): url = f"{WEBHOOK_URL}im.recent.get.json" response = requests.get(url) return response.json()

Функция для сохранения сообщений в JSON

def save_messages_to_json(messages, filename): with open(filename, "w", encoding="utf-8") as f: json.dump(messages, f, indent=2, ensure_ascii=False)

Функция для сохранения сообщений в TXT

def save_messages_to_txt(messages, filename): with open(filename, "w", encoding="utf-8") as f: for message in messages: author_id = message.get("author_id", "Unknown") date = message.get("date_create", "Unknown date") text = message.get("message", "") f.write(f"{date} - Author {author_id}: {text}\n") if 'files' in message: for file in message['files']: file_name = file.get('name', 'unnamed_file') file_url = file.get('url_download', '') f.write(f"[Файл] {file_name}: {file_url}\n") f.write("-"*50 + "\n")

Функция для скачивания файлов с прогресс-баром

def download_files(messages, folder_name): os.makedirs(folder_name, exist_ok=True) file_count = sum(len(message.get('files', [])) for message in messages) if file_count == 0: return progress_bar = st.progress(0) downloaded = 0
...
def zip_folder(folder_name, zip_name): with ZipFile(zip_name, 'w') as zipf: for root, dirs, files in os.walk(folder_name): for file in files: zipf.write(os.path.join(root, file), os.path.relpath(os.path.join(root, file), folder_name))

Основной процесс

if name == "main": st.title("Экспорт истории чатов Bitrix24")
